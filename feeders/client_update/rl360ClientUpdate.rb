...
File.open(csv_file, 'r') do |filehandle|
  filehandle.each do |line|
      
    rowno = rowno + 1
    row = line.split(',')
    row.each { |col| col.strip! }
    next unless (row[0])    # Skip empty Rows
    next if (row[0] == "")  # Skip empty Rows
    ...
    units_1 = 0.0 unless (units_1)
    units_1 = 0.0 if (units_1 == "")
    units_2 = 0.0 unless (units_2)
    units_2 = 0.0 if (units_2 == "")
